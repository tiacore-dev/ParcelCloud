...
export const TemplatesTable = (props: TemplatesTableProps) => {
  ...
  const templatesColumns = isMobile()
    ? templatesMobileColumns
    : templatesDesktopColumns;
  const columns = sort
    ? [{ key: "sort" }, ...templatesColumns]
    : templatesColumns;

  const setTemplatesKey = React.useCallback((param: setTemplatesKeyDto) => {
    useApi<ITemplate[], setTemplatesKeyDto>("templatesKey", "set", param)
      .then((templates) => {
  ...
  const onDragEnd = ({ active, over }: DragEndEvent) => {
    if (over && active.id !== over.id) {
      ...
      const movedTemplate = arrayMove(templates, activeIndex, overIndex).map(
        (template) => {
