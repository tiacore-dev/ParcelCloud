...
const pricesSlice = createSlice({
  ...
  initialState,
  reducers: {
    setSendCity: (state: IPricesState, action: { payload: string }) => {
      state.sendCity = action.payload;
    },
    setRecCity: (state: IPricesState, action: { payload: string }) => {
      state.recCity = action.payload;
    },
    settMax: (state: IPricesState, action: { payload: number }) => {
    ...
    getPricesFailure: (state: IPricesState) => {
      state.loading = false;
      state.loaded = false;
    },
    getPricesSuccess: (
      state: IPricesState,
      action: { payload: GetParcelResponce },
    ) => {
      state.loading = false;
      state.loaded = true;
